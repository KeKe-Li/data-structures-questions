...
|           25                |     [Mysql查询条件有两个独立的索引a和b同时，Mysql使用谁](#Mysql查询条件有两个独立的索引a和b同时，Mysql使用谁)            |   
...
#### 25. Mysql查询条件有两个独立的索引a和b同时，Mysql使用谁

这里需要看索引a和b，哪个索引区分度大，筛选更快就使用哪个。
...
* 一致性（Consistency）：每次请求都可以得到最新的数据。
