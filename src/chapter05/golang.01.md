...
- [1. Golang中除了加Mutex锁以外还有哪些方式安全读写共享变量？](#1.Golang中除了加Mutex锁以外还有哪些方式安全读写共享变量？)
- [2. 无缓冲Chan的发送和接收是否同步?](#2.无缓冲Chan的发送和接收是否同步?)
- [3. Golang语言的并发机制以及它所使用的CSP并发模型．](#3.Golang语言的并发机制以及它所使用的CSP并发模型．)
...
在创建好的`user`表中添加普通索引:
```sql
> ALTER TABLE `user` ADD KEY `idx-user_id` (`user_id`);
```

* 唯一索引

在创建好的`user`表中添加单一的唯一索引:

```sql
> ALTER TABLE `user` ADD UNIQUE KEY `uniq-user_id` (`user_id`);
```

在创建好的`user`表中添加复合的唯一索引:
```sql
> ALTER TABLE `user` ADD UNIQUE KEY `uniq-user_id-code` (`user_id`, `code`);
```
...
在创建好的`user`表中添加组合索引:
```sql
> ALTER TABLE `user` ADD KEY `idx-user_id-code` (`user_id`,`code`);
```
